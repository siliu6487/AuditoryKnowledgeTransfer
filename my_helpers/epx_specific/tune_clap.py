...
class ClapPiplineAudio(TransferPipeline):
    def __init__(self, context_dict: dict, audio_data_name: str, text_data_name: str, true_zero_shot: bool):
    ...
    def learn_encoder(self, hyparams, one_batch) -> dict:
        ...
        print(f"encoder train size: {len(source_dataset)}")
    ...
    def test_classifier(self, test_obj_list: None or list, use_enc: bool) -> dict:
        if test_obj_list is None:
            test_obj_list = self.full_obj_list
        ...
        for batch in test_dataloader:
            ...
            if use_enc:
                with torch.no_grad():
                    audio_enc = self.audio_encoder(audio_data)
