...
prev_test_fold = 0  # 👈 defatul 0 use this to skip used random seed for object sampling
...
context_dict, hyparams = set_default_context_hyparams(
    ...
    epoch_encoder=epoch_encoder, epoch_classifier=0  # we don't need to train clf
)
...
test_result = pipeline.test_classifier(test_obj_list=full_obj_list, use_enc=fine_tune)    # 0-shot
...
for i in range(num_test_fold):
    ...
    print(f"test classifier...")
    test_result = pipeline.test_classifier(test_obj_list=test_obj_lists[i], use_enc=fine_tune)
    print(f"test accuracy: {test_result['accuracy']}")
    print(f"test_obj_list: {test_result['test_obj_list']}")
    print(f"all_truth: {test_result['all_truth']}")
    print(f"all_pred: {test_result['all_pred']}")
